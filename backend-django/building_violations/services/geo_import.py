...
from ..models import GovtLandParcel, LandLayerUpload
from .geo import geojson_bbox
...
# ---------------------------------------------------------------- import
def import_layer(upload: LandLayerUpload, data: bytes, filename: str) -> LandLayerUpload:
    ...
    prev = LandLayerUpload.objects.filter(layer_key=upload.layer_key, active=True).exclude(pk=upload.pk).order_by("-version").first()
    if prev:
        upload.version = prev.version + 1
        upload.replaces = prev
        prev.active = False
        prev.save(update_fields=["active"])
        GovtLandParcel.objects.filter(layer_upload=prev).update(active=False)
    n = skipped = 0
    log = []
    for geom, props in feats:
        ...
        from ..models import LandOwningAgency
        if agency not in dict(LandOwningAgency.choices):
