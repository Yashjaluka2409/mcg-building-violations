    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        <Card>
          ...
          {(c.use_observed || c.occupants_total != null) && <Muted>{[c.use_observed && `Use: ${c.use_observed}`, c.occupants_total != null && `Occupants: ${c.occupants_total}`, c.occupants_senior_citizens != null && `senior citizens ${c.occupants_senior_citizens}`, c.occupants_children != null && `children ${c.occupants_children}`, c.occupants_women != null && `women ${c.occupants_women}`].filter(Boolean).join(" · ")}</Muted>}
        ...
        {c.litigation_status && c.litigation_status !== "NONE" && <Card style={{ borderLeftWidth: 4, borderLeftColor: c.litigation_status === "STAYED" ? "#334155" : colors.muted }}><CardTitle>{c.litigation_status === "STAYED" ? "⚖ STAY IN FORCE" : c.litigation_status === "APPEAL_PENDING" ? "⚖ Appeal pending" : "⚖ Litigation decided"}</CardTitle><Text>{String(c.litigation_authority || "").replace(/_/g, " ")}{c.stay_until ? ` · stay till ${String(c.stay_until)}` : c.litigation_status === "STAYED" ? " · until further orders" : ""}</Text>{c.next_hearing_on && <Muted>Next hearing {String(c.next_hearing_on)}</Muted>}{c.litigation_status === "STAYED" && <Text style={{ color: colors.danger, fontWeight: "700", marginTop: 6 }}>Do NOT execute demolition / sealing. Stay order is on the case file (portal → Litigation).</Text>}</Card>}
