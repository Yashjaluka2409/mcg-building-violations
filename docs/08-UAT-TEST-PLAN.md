...
| 42 | Paper order on record | JC 9000000003 → Orders before the system → Add an order (order no, date, PID, owner, served on, scan) | Case created at "Served - compliance period running" (or "Execution due" if the period is over); order shows "paper order (pre-system)" with the original number; scan under Evidence → Legacy order; audit shows LEGACY_IMPORT then LEGACY_STATUS |
| 43 | Register upload | Download the template, fill 3 rows (one with a bad date), upload | 2 imported, 1 row error listed with the reason; re-upload of the same numbers is skipped as "already on record" |
| 44 | Update from the file | Register → Update status → Executed on a past date with file reference | Status EXECUTED, execution record with that date, event remarks carry the reference; JE app shows the case with the normal "verify & close" flow available to the JC |
| 45 | Stay on an old order | Update status → Stayed, High Court, stay until date | Case shows "Stayed in appeal"; later "Served - compliance running" update records the stay as vacated |
