  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-3">
        <div><h1 className="page-title">{inbox ? t("nav.inbox") : t("nav.cases")}</h1><p className="page-sub">{inbox ? `Cases waiting for action by ${user?.role_label || user?.role}` : "All violation cases in your jurisdiction"}</p></div>
        <div className="flex-1" />
        {(user?.can_create_case ?? ["JE", "AE", "FIELD_STAFF", "ADMIN"].includes(user?.role || "")) && <button className="btn-primary" onClick={() => nav("/cases/new")}>+ {t("nav.new_case")}</button>}
      </div>
          ...
          <table className="table">
            ...
            <tbody>{q.data.results.map((c) => (
              <tr key={c.id} className="hover:bg-primary-50/40 cursor-pointer" onClick={() => nav(`/cases/${c.id}`)}>
                ...
                <td><StatusBadge status={c.status} display={wf.hi ? c.status_display_hi || c.status_display : c.status_display} /><div className="text-[11px] text-light-text-muted mt-1">{c.days_in_stage} d in stage</div></td>
                <td className="text-xs">{c.current_owner_label || c.current_owner_role}<div className="text-light-text-muted">{c.current_owner?.name || (c.current_owner_role === "JE" ? c.reported_by?.name : c.current_owner_role === "AE" ? c.assigned_ae?.name : c.assigned_jc?.name) || "-"}</div></td>
