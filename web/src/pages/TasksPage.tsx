      ...
      <div className="space-y-3">
        ...
        <div className="grid md:grid-cols-2 gap-3">
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2"><Field label="Owner"><input className="input" value={f.owner_name || ""} onChange={(e) => setF({ ...f, owner_name: e.target.value })} /></Field><Field label="Mobile"><input className="input" value={f.owner_mobile || ""} onChange={(e) => setF({ ...f, owner_mobile: e.target.value })} /></Field><Field label="Category"><select className="input" value={f.category} onChange={(e) => setF({ ...f, category: e.target.value })}>{CATS.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select></Field><Field label="Priority"><select className="input" value={f.priority} onChange={(e) => setF({ ...f, priority: e.target.value })}>{["LOW", "NORMAL", "HIGH", "URGENT"].map((x) => <option key={x}>{x}</option>)}</select></Field><Field label="Assign to JE" hint="blank = JE of the ward automatically"><select className="input" value={f.assigned_to || ""} onChange={(e) => setF({ ...f, assigned_to: e.target.value || null })}><option value="">Auto (by ward)</option>{jes.data?.map((o) => <option key={o.user_id} value={o.user_id}>{o.name}</option>)}</select></Field><Field label="Days to complete"><input type="number" className="input" value={f.due_days} onChange={(e) => setF({ ...f, due_days: Number(e.target.value) })} /></Field></div>
