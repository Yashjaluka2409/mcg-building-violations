...
export default function OfficersPage() {
  ...
  const ROLES = wf.roles.filter((r) => r.active).map((r) => r.code);   // role catalogue from Administration → Hierarchy
  const authorityRoles = wf.slotRoles("AUTHORITY");
  ...
  const isJC = (user?.slot ? user.slot === "AUTHORITY" : user?.role === "JC") && !canManage;
    ...
    <div className="space-y-4">
      ...
      <Modal open={!!edit} onClose={() => setEdit(null)} title={edit?.id ? `${edit.display_name || "Officer"} (${edit.role})` : "New officer login"} wide>{edit && <div className="space-y-3">
        ...
        {(tab === "profile" || !edit.id) && <>
          ...
          {authorityRoles.includes(edit.role) && <div className="grid grid-cols-2 gap-2"><Field label="Delegation order no. (s.401(2)) - printed on notices"><input className="input" value={edit.delegation_order_no || ""} onChange={(e) => setEdit({ ...edit, delegation_order_no: e.target.value })} /></Field><Field label="Delegation order date"><input type="date" className="input" value={edit.delegation_order_date || ""} onChange={(e) => setEdit({ ...edit, delegation_order_date: e.target.value })} /></Field></div>}
...
function JurisdictionFields({ edit, setEdit, zones, wardOpts, divisions, officersList }: any) {
  ...
  const wf = useWorkflowConfig();
  const supervisors = officersList.filter((o: any) => o.id !== edit.id && !wf.slotRoles("REPORTER").includes(o.role) && !["JC_CLERK", "FIELD_STAFF", "BRANCH_OFFICER", "GIS_LAB", "VIEWER"].includes(o.role));
  return (
