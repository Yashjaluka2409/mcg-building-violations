...
from app.models.building_violations import AppAttestKey

PLAY_SCOPE = "https://www.googleapis.com/auth/playintegrity"
APP_ATTEST_NONCE_OID = "1.2.840.113635.100.8.2"


def verify(db: Session, att: dict, *, user, consume_nonce) -> tuple[str, dict]:
    ...
    try:
        ...
        if kind == "app_attest":
            ...
            if att.get("attestation"):
                return verify_app_attest_attestation(db, att["key_id"], att["attestation"], nonce, user)
            return verify_app_attest_assertion(db, att["key_id"], att.get("assertion", ""), nonce, user)
...
def _google_access_token(sa_path: str) -> str:
    ...
    claims = _b64url(json.dumps({"iss": sa["client_email"], "scope": PLAY_SCOPE, "aud": sa["token_uri"], "iat": t, "exp": t + 3600}).encode())
...
def verify_play_integrity(token: str, nonce: str) -> tuple[str, dict]:
    sa_path = settings.BVMS_PLAY_INTEGRITY_SA_JSON
    package = settings.BVMS_ANDROID_PACKAGE
    ...
    if settings.BVMS_PLAY_INTEGRITY_REQUIRE_PLAY_RECOGNIZED and app.get("appRecognitionVerdict") != "PLAY_RECOGNIZED":
...
def _app_id_hash() -> bytes | None:
    team, bundle = settings.BVMS_APPLE_TEAM_ID, settings.BVMS_IOS_BUNDLE_ID
    return hashlib.sha256(f"{team}.{bundle}".encode()).digest() if team and bundle else None


def _load_root():
    path = settings.BVMS_APP_ATTEST_ROOT_CA
    if not path:
        return None
...
def _verify_chain(certs, root):
    ...
    for i, cert in enumerate(chain[:-1]):
        ...
        if not (cert.not_valid_before_utc <= t <= cert.not_valid_after_utc):
...
def verify_app_attest_attestation(db: Session, key_id: str, attestation_b64: str, nonce: str, user) -> tuple[str, dict]:
    import cbor2
    from cryptography import x509
    ...
    env = settings.BVMS_APP_ATTEST_ENV
    ...
    row = db.get(AppAttestKey, key_id)
    if row is None:
        row = AppAttestKey(key_id=key_id)
        db.add(row)
    row.user_id, row.public_key_pem, row.counter, row.environment = user.id, pem, 0, env
    db.flush()
    return "VALID", {"key_id": key_id, "environment": env, "first_use": True}


def verify_app_attest_assertion(db: Session, key_id: str, assertion_b64: str, nonce: str, user) -> tuple[str, dict]:
    import cbor2
    from cryptography.hazmat.primitives import hashes, serialization
    from cryptography.hazmat.primitives.asymmetric import ec
    ...
    key = db.query(AppAttestKey).filter(AppAttestKey.key_id == key_id, AppAttestKey.user_id == user.id).first()
    if not key:
    ...
    key.counter, key.last_used_at = ad["counter"], now()
    db.flush()
