...
def run_blocking(fn, *args, **kwargs):
    """pyHanko drives signing with asyncio.run(); when we are already inside the server's event loop (the
    business logic runs in AsyncSession.run_sync on the loop thread) it must run on a thread of its own."""
    try:
...
class LocalP12Signer:
    ...
    def sign(self, pdf_bytes: bytes, reason: str, signer_name: str, location: str = "Gurugram") -> SignResult:
        return run_blocking(self._sign, pdf_bytes, reason, signer_name, location)

    def _sign(self, pdf_bytes: bytes, reason: str, signer_name: str, location: str = "Gurugram") -> SignResult:
        try:
...
class PKCS11Signer(LocalP12Signer):
    ...
    def sign(self, pdf_bytes, reason, signer_name, location="Gurugram"):
        return run_blocking(self._sign_token, pdf_bytes, reason, signer_name, location)

    def _sign_token(self, pdf_bytes, reason, signer_name, location="Gurugram"):
        cfg = settings.signer
        try:
...
def get_signer():
    backend = settings.signer.get("BACKEND", "local")
