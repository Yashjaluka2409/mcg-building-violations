...
from app.models.building_violations import InspectionBatch, InspectionTask, MediaAttachment, OfficerProfile, Role, TaskStatus, Ward, Zone
...
def geofence_m(db: Session, task: InspectionTask | None = None) -> int:
    return int(task.geofence_m if task and task.geofence_m else access.get_setting(db, "inspection_geofence_m", 100))


def pick_je(db: Session, ward: Ward | None, zone=None):
...
def create_task(db: Session, user, *, pid: str = "", address: str = "", latitude=None, longitude=None, category="VERIFICATION", instructions="", priority="NORMAL",
    ...
    if not access.has_perm(db, user, "TASKS_ASSIGN"):
    ...
    if pid and lookup_pid and (latitude is None or not address):
        ...
        if rec:
            ...
            if ward is None and rec.ward_no and str(rec.ward_no).strip().isdigit():
                ward = db.query(Ward).filter(Ward.number == int(str(rec.ward_no).strip())).first()
    ...
    if latitude is not None and longitude is not None and ward is None:
        ward = ward_for_point(db, latitude, longitude)
    zone = zone or (ward.zone if ward else None)
    if assigned_to is None:
        assigned_to = pick_je(db, ward, zone)
    days = due_days if due_days is not None else 7
    t = InspectionTask(
        batch=batch, category=category, pid=pid or "", pid_snapshot=snap, address=address or "", owner_name=owner_name or "", owner_mobile=(owner_mobile or "")[:15],
        latitude=latitude, longitude=longitude, ward=ward, zone=zone, instructions=instructions or "", priority=priority, created_by=user,
        assigned_to=assigned_to, assigned_at=now() if assigned_to else None, due_at=now() + timedelta(days=int(days)),
        status=TaskStatus.ASSIGNED if assigned_to else TaskStatus.UNASSIGNED, related_case=related_case, geofence_m=geofence_m(db))
    db.add(t)
    db.flush()
    if assigned_to:
        notify_user(db, assigned_to, None, f"Inspection assigned: {t.pid or t.address[:40]}", f"{t.get_category_display()} · {t.instructions[:120]} · due {t.due_at:%d-%m-%Y}")
    return t


def assign_task(db: Session, task: InspectionTask, user, assigned_to, remarks=""):
    if not access.has_perm(db, user, "TASKS_ASSIGN"):
        raise WorkflowError("You do not have the TASKS_ASSIGN permission", 403)
    if task.status in (TaskStatus.VIOLATION_RECORDED, TaskStatus.NO_VIOLATION, TaskStatus.NOT_FOUND, TaskStatus.CANCELLED):
        raise WorkflowError("This inspection is already closed")
    task.assigned_to, task.assigned_at = assigned_to, now()
    task.status = TaskStatus.ASSIGNED
    db.flush()
    notify_user(db, assigned_to, None, f"Inspection assigned: {task.pid or task.address[:40]}", remarks or task.instructions[:120])
    return task
...
def start_task(db: Session, task: InspectionTask, user, *, latitude, longitude, accuracy_m=None):
    """The field officer presses 'Start inspection' on site. Refused outside the geofence."""
    if not access.has_perm(db, user, "TASKS_EXECUTE") and task.assigned_to_id != user.id:
    ...
    if task.status not in (TaskStatus.ASSIGNED, TaskStatus.UNASSIGNED, TaskStatus.IN_PROGRESS):
        raise WorkflowError("This inspection is already closed")
    ...
    if task.latitude is None:
        ...
        task.ward = task.ward or ward_for_point(db, latitude, longitude)
    task.status = TaskStatus.IN_PROGRESS
    task.started_at = now()
    task.start_latitude, task.start_longitude, task.start_distance_m = latitude, longitude, dist
    if not task.assigned_to_id:
        task.assigned_to, task.assigned_at = user, now()
    db.flush()
    return task


def complete_task_no_violation(db: Session, task: InspectionTask, user, *, outcome: str, remarks: str, media_ids=None, latitude=None, longitude=None):
    if task.assigned_to_id != user.id and not access.has_perm(db, user, "TASKS_ASSIGN"):
        raise WorkflowError("This inspection is not assigned to you", 403)
    ...
    if outcome not in (TaskStatus.NO_VIOLATION, TaskStatus.NOT_FOUND):
    ...
    if latitude is not None:
        dist = distance_to_task(task, latitude, longitude)
        if dist is not None and dist > geofence_m(db, task):
            raise WorkflowError(f"You are {dist:.0f} m from the property; the report must be filed on site")
    for m in db.query(MediaAttachment).filter(MediaAttachment.id.in_(uuids(media_ids))):
        ...
        if m.latitude is not None and task.latitude is not None:
            ...
            m.geotag_verified = float(m.distance_from_case_m) <= geofence_m(db, task)
    task.status, task.completed_at, task.outcome_remarks = outcome, now(), remarks or ""
    db.flush()
    notify_user(db, task.created_by, None, f"Inspection closed - {task.get_status_display()}: {task.pid or task.address[:40]}", (remarks or "")[:200])
    return task


def cancel_task(db: Session, task: InspectionTask, user, remarks=""):
    if not access.has_perm(db, user, "TASKS_ASSIGN"):
        raise WorkflowError("You do not have the TASKS_ASSIGN permission", 403)
    task.status, task.completed_at, task.outcome_remarks = TaskStatus.CANCELLED, now(), remarks or ""
    db.flush()
    return task


def link_case_to_task(db: Session, task: InspectionTask, case, inspector_lat=None, inspector_lng=None):
    """Called by workflow.create_case when a case is recorded from a task. Enforces the geofence."""
    if task.status in (TaskStatus.VIOLATION_RECORDED, TaskStatus.CANCELLED):
        raise WorkflowError("This inspection task is already closed")
    if access.get_setting(db, "require_geofence_for_task_inspection", True):
        ...
        if dist is not None and dist > geofence_m(db, task):
            raise WorkflowError(f"You are {dist:.0f} m from the property; the inspection must be recorded within {geofence_m(db, task)} m")
        case.inspector_distance_m = dist
    task.status, task.completed_at = TaskStatus.VIOLATION_RECORDED, now()
    case.task = task
    db.flush()
    notify_user(db, task.created_by, case, f"Violation recorded on pushed inspection {task.pid or task.address[:40]}", case.case_no)


def bulk_create_from_rows(db: Session, user, rows: list[dict], *, title: str, category: str, instructions: str, due_days: int, source_file: str | None = None,
                          default_assignee=None, lookup_pid=True) -> InspectionBatch:
    batch = InspectionBatch(title=title, category=category, created_by=user, instructions=instructions or "", due_at=now() + timedelta(days=due_days), source_file=source_file or "")
    db.add(batch)
    db.flush()
    errors = []
    n = 0
    for i, r in enumerate(rows, start=2):
        ...
        if r.get("ward_number") not in (None, ""):
            try:
                ward = db.query(Ward).filter(Ward.number == int(r["ward_number"])).first()
            except (TypeError, ValueError):
                ward = None
        assignee = default_assignee
        if r.get("assign_to_mobile"):
            p = db.query(OfficerProfile).filter(OfficerProfile.mobile == str(r["assign_to_mobile"]).strip()[-10:], OfficerProfile.active == True).first()  # noqa: E712
            assignee = p.user if p else assignee
        try:
            with db.begin_nested():
                create_task(db, user, pid=pid, address=addr, latitude=lat, longitude=lng, category=str(r.get("category") or category), instructions=str(r.get("instructions") or instructions),
