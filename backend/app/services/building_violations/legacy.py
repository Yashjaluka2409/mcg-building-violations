...
from app.core.timeutil import aware, localdate, make_aware_local, now
from app.models.building_violations import (Appeal, AppealStatus, CaseStatus as S, CaseViolation, ExecutionMode, ExecutionRecord, LegacyOrderBatch, Notice, OrderType, Role,
                         SignatureStatus, ViolationCase, ViolationType, Ward)
...
from app.services.building_violations.geo import ward_for_point
from app.services.building_violations.numbering import next_case_no, verification_code
from app.services.building_violations.workflow import _set_status, attach_media
...
def _dt(d, hour=10):
    ...
    if isinstance(d, datetime):
        return aware(d) if d.tzinfo else make_aware_local(d)
    if isinstance(d, str):
        d = date.fromisoformat(d[:10])
    return make_aware_local(datetime.combine(d, time(hour, 0)))


def _authorize(db: Session, user):
    prof = getattr(user, "bvms_profile", None)
    if not prof:
        raise WorkflowError("No officer profile", 403)
    if prof.role in access.MANAGEMENT_ROLES or access.has_perm(db, user, "LEGACY_ORDERS_MANAGE"):
        return prof
    raise WorkflowError("Only the JC office / module administrator may record orders issued outside the system", 403)


def _ward(db: Session, d):
    w = d.get("ward")
    if w is not None and hasattr(w, "id"):
        return w
    if w:
        return db.get(Ward, int(w))
    if d.get("ward_number"):
        return db.query(Ward).filter(Ward.number == int(d["ward_number"])).first()
    if d.get("latitude") and d.get("longitude"):
        return ward_for_point(db, d["latitude"], d["longitude"])
    return None
...
def derive_status(d) -> str:
    ...
    if d.get("executed_on"):
        return S.COMPLIED if d.get("execution_mode") == ExecutionMode.OWNER_SELF else S.EXECUTED
    if d.get("stay_granted") or d.get("stay_until"):
        return S.APPEAL_STAY
    if d.get("served_on"):
        due = d.get("_compliance_due_at")
        return S.EXECUTION_DUE if due and due < now() else S.ORDER_SERVED
...
# single order
# ---------------------------------------------------------------------------
def import_order(db: Session, user, d: dict, request=None, batch: LegacyOrderBatch | None = None) -> ViolationCase:
    prof = _authorize(db, user)
    order_no = str(d.get("order_no") or "").strip()
    if not order_no:
        raise WorkflowError("The original order number is required")
    if db.query(Notice.id).filter(Notice.notice_no == order_no).first():
    ...
    ot = db.get(OrderType, d.get("order_type") or "DEMOLITION_ORDER_261")
    if not ot or not ot.active or ot.kind != "ORDER":
    ...
    case = ViolationCase(
        case_no=next_case_no(db, zone.code if zone else None), status=S.DRAFT, status_changed_at=now(), source=LEGACY_SOURCE, legacy_reference=str(d.get("legacy_reference") or "")[:120],
    ...
    for i, v in enumerate(codes):
        ...
        vt = db.get(ViolationType, code)
        if not vt:
            raise WorkflowError(f"Unknown violation code {code}")
        case.violations.append(CaseViolation(violation_type=vt, remarks=(v.get("remarks", "") if isinstance(v, dict) else "") or "",
                                             details=(v.get("details", {}) if isinstance(v, dict) else {}) or {}, is_primary=(i == 0)))
    db.flush()
    ...
    n = Notice(
        ...
        signature_status=SignatureStatus.UNSIGNED, signer_name=signer, verification_code=verification_code(),
        context_snapshot={"legacy": {k: (str(v) if isinstance(v, (date, datetime, Decimal)) else jsonable(v)) for k, v in d.items() if not k.startswith("_") and k != "media_ids"}},
    ...
    if d.get("media_ids"):
        attach_media(db, case, d["media_ids"], user, kind="LEGACY_ORDER", notice=n)

    record_event(db, case, "LEGACY_IMPORT", actor=user, from_status="", to_status=S.DRAFT, request=request,
    ...
    _apply_history(db, case, n, user, d, status, request)
    return case


def _apply_history(db: Session, case, n, user, d, status, request, remarks=""):
    ...
    if d.get("executed_on") or status in (S.EXECUTED, S.COMPLIED):
        ex_on = _dt(d.get("executed_on")) or now()
        mode = d.get("execution_mode") or (ExecutionMode.OWNER_SELF if status == S.COMPLIED else ExecutionMode.CORPORATION)
        ...
        ex = ExecutionRecord(case=case, order=n, action=action, mode=mode, executed_on=ex_on, cost_incurred_inr=d.get("cost_incurred_inr") or None,
        ...
        if d.get("media_ids") and status in (S.EXECUTED, S.COMPLIED) and case.status != S.DRAFT:
            attach_media(db, case, d["media_ids"], user, kind="EXECUTION" if mode == "CORPORATION" else "COMPLIANCE")
    ...
    if d.get("appeal_authority"):
        ...
        ap = Appeal(case=case, order=n, authority=d["appeal_authority"], authority_other=str(d.get("authority_other") or ""),
                    filed_on=d.get("appeal_filed_on") or d.get("stay_order_date") or localdate(), appeal_no=str(d.get("appeal_no") or ""),
                    appellant_name=str(d.get("appellant_name") or case.owner_name), status=AppealStatus.STAYED if stay else AppealStatus.PENDING,
    ...
    if status in (S.CLOSED, S.REGULARISED, S.DROPPED):
        case.closed_at = _dt(d.get("closed_on"), 17) or now()
    ...
    db.flush()
    _set_status(db, case, status, user, "LEGACY_STATUS", request, remarks or "Status as per the paper record at the time of import", payload=payload,
...
# ---------------------------------------------------------------------------
def update_status(db: Session, case: ViolationCase, user, d: dict, request=None) -> ViolationCase:
    ...
    if status in (S.ORDER_SERVED, S.EXECUTION_DUE):
        ...
        if n and not n.served_at:
            ...
            db.flush()
        if d.get("media_ids"):
            attach_media(db, case, d["media_ids"], user, kind="ORDER_DELIVERY", notice=n)
    ...
    if status == S.APPEAL_STAY and d.get("media_ids"):
        attach_media(db, case, d["media_ids"], user, kind="STAY_ORDER")
    ...
    if status in (S.ORDER_SERVED, S.ORDER_ISSUED) and case.litigation_status == "STAYED":
        ...
        for ap in db.query(Appeal).filter(Appeal.case_id == case.id, Appeal.status == AppealStatus.STAYED):
            ap.status, ap.stay_granted = AppealStatus.STAY_VACATED, False
        case.litigation_status, case.stay_until = "DECIDED", None
    _apply_history(db, case, n, user, d, status, request, remarks=remarks)
    return case
...
def row_to_data(r: dict) -> dict:
    g = lambda k: (str(r.get(k)).strip() if r.get(k) is not None else "")  # noqa: E731
...
def import_rows(db: Session, user, rows: list[dict], *, title: str, source_file: str | None = None, request=None, order_reference: str = "") -> LegacyOrderBatch:
    _authorize(db, user)
    batch = LegacyOrderBatch(title=title or f"Legacy orders {localdate()}", created_by=user, source_file=source_file or "", total_rows=len(rows), created_at=now())
    db.add(batch)
    db.flush()
    errors, n = [], 0
    for i, r in enumerate(rows, start=2):
        try:
            ...
            with db.begin_nested():
                import_order(db, user, d, request=request, batch=batch)
            n += 1
    ...
    db.flush()
    access.log_admin(db, user, "LEGACY_IMPORT", "LegacyOrderBatch", batch.id, after={"imported": n, "errors": len(errors)}, order_reference=order_reference,
                     remarks=title, request=request)
    return batch


def summary(db: Session, criteria: list) -> dict:
    """Counts by status for the cases matching ``criteria`` (the caller's jurisdiction scope)."""
    rows = db.query(ViolationCase.status, func.count(ViolationCase.id)).filter(*criteria, ViolationCase.source == LEGACY_SOURCE).group_by(ViolationCase.status).all()
    by_status = {st: c for st, c in rows}
    open_states = (S.ORDER_ISSUED, S.ORDER_SERVED, S.EXECUTION_DUE, S.APPEAL_STAY)
    return {"total": sum(by_status.values()), "open": sum(by_status.get(s, 0) for s in open_states), "by_status": by_status,
            "execution_due": by_status.get(S.EXECUTION_DUE, 0), "stayed": by_status.get(S.APPEAL_STAY, 0),
            "batches": db.query(func.count(LegacyOrderBatch.id)).scalar() or 0}
